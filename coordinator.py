...
class FelicityCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> Dict[str, Any]:
        try:
            configured_inverter_sn = self.entry.data.get("device_sn") or None
            configured_battery_sn = self.entry.data.get("battery_sn") or None

            list_raw = await self.api.get_device_list()
            list_data = self._extract_device_list(list_raw)

            devices_by_sn = self._index_devices_by_sn(list_data)

            inverter_list = self._find_inverter(list_data, configured_inverter_sn)
            battery_list = self._find_battery(list_data, configured_battery_sn)

            inverter_sn = self._first(
                inverter_list.get("deviceSn"),
                configured_inverter_sn,
            )

            battery_sn = self._first(
                battery_list.get("deviceSn"),
                configured_battery_sn,
            )

            _LOGGER.warning("Felicity inverter_sn: %s", inverter_sn)
            _LOGGER.warning("Felicity battery_sn: %s", battery_sn)

            inverter = await self._build_device_data(
                inverter_sn,
                inverter_list,
            )

            battery = await self._build_device_data(
                battery_sn,
                battery_list,
            )
            ...
            legacy_snapshot = self._build_legacy_snapshot(inverter, battery)
    ...
    async def _build_device_data(
        self,
        device_sn: str | None,
        list_device: Dict[str, Any],
    ) -> Dict[str, Any]:
        if not device_sn:
            return dict(list_device or {})

        snapshot = await self._safe_snapshot(device_sn)
        basic = await self._safe_basic(device_sn)
    ...
    async def _safe_snapshot(self, device_sn: str) -> Dict[str, Any]:
        try:
            ...
            if raw.get("status") in (400, 401, 403, 404, 500):
                _LOGGER.warning("Felicity snapshot HTTP error for %s: %s", device_sn, raw)
                return {}

            if raw.get("code") not in (None, 200):
                _LOGGER.warning("Felicity snapshot API error for %s: %s", device_sn, raw)
                return {}
            ...
            _LOGGER.warning("Felicity snapshot failed for %s: %s", device_sn, err)
            return {}
    ...
    def _index_devices_by_sn(self, device_list):
        result = {}
    ...
    def _find_device(self, device_list, device_sn):
        ...
        for dev in device_list:
            ...
            if str(dev.get("deviceSn")) == str(device_sn):
                return dev

        return {}

    def _find_inverter(self, device_list, configured_sn=None):
        ...
        # Manuelle SN nur als Fallback
        return self._find_device(device_list, configured_sn)

    def _find_battery(self, device_list, configured_sn=None):
        # Auto-Erkennung zuerst
        for dev in device_list:
            if not isinstance(dev, dict):
                continue
    ...
    def _normalize_inverter(self, inverter: Dict[str, Any]) -> None:
        ...
        inverter["pv1Power"] = self._first(
            inverter.get("pvPower"),
            inverter.get("pv1Power"),
        )
        ...
        inverter["alarmText"] = self._first(
            inverter.get("failCode"),
            "",
        )
